        ...
        <header className="h-16 border-b border-neutral-800 bg-black/50 backdrop-blur-md flex items-center justify-between px-6 sticky top-0 z-50">
            <div className="flex items-center gap-6">
                ...
                {isAdmin && (
                    <Link
                        to="/admin"
                        className="hidden md:flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-blue-500 hover:bg-white/10 transition-all uppercase tracking-widest"
                    >
                        Admin Panel
                    </Link>
                )}
                ...
                {authLoading ? (
                    ...
                    <div className="flex items-center gap-4">
                        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-neutral-800/50 rounded-full border border-neutral-700 text-xs text-neutral-400">
                            ...
                            <span className={`ml-2 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase ${isAdmin ? 'bg-blue-600 text-white' : 'bg-neutral-700 text-neutral-500'}`}>
                                {isAdmin ? 'Admin' : 'User'}
                            </span>
                        </div>
                        <button
